...
// import nodesController from './nodeController';
export class App {
    ...
    async config(socketIO?: Server) {
        ...
        //         var allNodeNames = allnames.trim().split('|')
        //         for (const nodeName in this.nodesPool.componentNodes) {
        //             let resultNode = allNodeNames.find(e => e.includes(nodeName));  
        //             const clonedNode = cloneDeep(this.nodesPool.componentNodes[nodeName]);
        //             if(resultNode != undefined && resultNode != null && resultNode.length > 0 ){
        //                 returnData.push(clonedNode);
        //             }else{
        //                 for(const node in allNodeNames){
        //                    var  data = clonedNode.baseClasses.find(e => e.includes(node));
        //                    if(data != undefined && data != null && data.length >0 ){
        //                     let {label} = clonedNode;
        //                     const i = returnData.findIndex(e => e.label === label);
        //                    if (i == -1) {
        ...
        // }
        // })
         // Get all possible connected nodes
       // this.app.get('/api/v1/connected-nodes/:allnames', nodesController.getNodeByAllNames)
